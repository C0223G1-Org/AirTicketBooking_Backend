...
public interface ITicketRepository extends JpaRepository<Ticket, Long> {
     ...
     * @author Nhàn NA
     */
    @Query(value = "select id_ticket as id, date_booking as dateBooking, name_passenger as namePassenger, name_route as nameRoute,name_departure as nameDeparture , name_destination as  nameDestination, time_departure as timeDeparture,r.date_departure as departureDate,price_ticket as priceTicket  from ticket t \n" +
            ...
            "where  s.flag_seat=1",nativeQuery = true)
    ...
    @Query(value = "update seat s\n" +
            ...
            "set s.flag_seat=0\n" +
            "where t.id_ticket=:id",nativeQuery = true)
     ...
     * @author Nhàn NA
     */
    @Query(value = "select  * from ticket where  ticket.id_ticket=:id",nativeQuery = true)
    ...
    @Query(value ="select id_ticket as id, date_booking as dateBooking, name_passenger as namePassenger, name_route as nameRoute,name_departure as nameDeparture , name_destination as  nameDestination, time_departure as timeDeparture,r.date_departure as departureDate,price_ticket as priceTicket  from ticket t \n" +
            ...
            "            join air_craft ac on ac.id_air_craft = r.id_air_craft\n" +
            "            join departure de on de.id_departure=r.id_departure\n" +
            "            where  s.flag_seat=1 and id_type_ticket=2 and  name_passenger like concat('%',:#{#ticketSearch.passenger},'%') and name_departure like concat('%',:#{#ticketSearch.departure},'%') and name_destination like concat('%',:#{#ticketSearch.destination},'%')  and " +
    ...
    @Query(value = "SELECT id_ticket AS id,date_booking AS dateBooking,name_passenger AS namePassenger,name_route AS nameRoute,\n" +
            "  name_departure AS nameDeparture,name_destination AS nameDestination,time_departure AS timeDeparture,r.date_departure as departureDate,price_ticket AS priceTicket\n" +
            "FROM\n" +
            "  ticket t\n" +
            "  JOIN seat s ON s.id_seat = t.seat_id_seat\n" +
            "  JOIN route r ON r.id_route = s.id_route\n" +
            ...
            "  JOIN type_seat ts ON ts.id_type_seat = s.id_type_seat\n" +
            "  JOIN air_craft ac ON ac.id_air_craft = r.id_air_craft\n" +
            "WHERE id_type_ticket=1 and s.flag_seat=1 and :#{#ticketSearch.destinationDate} IN (\n" +
    ...
    @Query(value = "select id_ticket as id, date_booking as dateBooking, name_passenger as namePassenger, name_route as nameRoute,name_departure as nameDeparture , name_destination as  nameDestination, time_departure as timeDeparture,r.date_departure as departureDate,price_ticket as priceTicket  from ticket t \n" +
            ...
            "join air_craft ac on ac.id_air_craft = r.id_air_craft\n" +
            "join departure de on de.id_departure=r.id_departure\n" +
            "where  s.flag_seat=1 and position_seat like concat('%',:#{#ticketSearch.chairCode},'%') and  name_passenger like concat('%',:#{#ticketSearch.passenger},'%') " +
